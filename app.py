...
/* Radio buttons, light theme with pill styling */
...
[data-testid="stDataFrame"] [role="row"]:hover {
    background: rgba(217,138,43,0.06) !important;
}

/* Smooth fade-in on load */
.block-container {
    animation: gciFade 0.5s ease-out;
}
@keyframes gciFade {
...
# ── MASTHEAD ──────────────────────────────────────────────────────
st.markdown("""
<div class="gci-eyebrow">Hormuz Crisis Intelligence &middot; Day 112</div>
...
with col_left:
    ...
    trend_df = pd.DataFrame([
        {"date": str(h["date"]), "score": h["score"]} for h in GCI_HISTORY
    ])
    chart = (
        alt.Chart(trend_df)
        .mark_line(color="#D98A2B", strokeWidth=2.5, point=alt.OverlayMarkDef(color="#B26B16", size=45))
        .encode(
            x=alt.X("date:N", title=None, axis=alt.Axis(labelColor="#6B8198", labelAngle=-40, domainColor="#D4E0EC", tickColor="#D4E0EC")),
            y=alt.Y("score:Q", title=None, scale=alt.Scale(domain=[60, 100]), axis=alt.Axis(labelColor="#6B8198", domainColor="#D4E0EC", tickColor="#D4E0EC", gridColor="#E6EDF4")),
            tooltip=[alt.Tooltip("date:N", title="Date"), alt.Tooltip("score:Q", title="GCI")],
        )
